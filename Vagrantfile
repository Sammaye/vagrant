...
Vagrant.configure("2") do |config|
  ...
  config.vm.box = "sammaye/ubuntu-18-04-100GB"
  ...
  config.vm.provision "shell", inline: <<-SHELL
    ...
    php7.1-curl \
    php7.1-gd \
	# This causes a race condition which means dpkg cannot lock files
    #php7.1-zip \
    php7.1-mysql \
    php7.1-mbstring \
    php7.1-xml \
    ...
    sed -i -e "s/www-data/vagrant/g" /etc/php/7.1/fpm/pool.d/www.conf
    ...
    cat > /etc/php/7.1/mods-available/mongodb.ini << 'EOF'
; configuration for php mongodb module
; priority=30
extension=mongodb.so
EOF
    ln -s /etc/php/7.1/mods-available/mongodb.ini /etc/php/7.1/fpm/conf.d/30-mongodb.ini
    ln -s /etc/php/7.1/mods-available/mongodb.ini /etc/php/7.1/cli/conf.d/30-mongodb.ini

	# Apply changes by restarting fpm
    ...
    mysqladmin -u root -p'root' password '' # Remove root password
